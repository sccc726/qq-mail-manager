...
from qqmail_core.folders import FolderError, parse_list_response, quote_mailbox
from qqmail_core.imap_uid import select_uid_fetch
from qqmail_core.mailref import MailRef, MailRefError, parse_uidvalidity
...
def extract_preview(message, max_len=150):
...
def _fetch(data, uid):
    return select_uid_fetch(data, uid)
...
def fetch_email_summary(mail, reference):
    ...
    status, data = mail.uid("FETCH", reference.uid, "(BODY.PEEK[] INTERNALDATE)")
    response = _fetch(data, reference.uid) if status == "OK" else None
    if response is None:
        raise MailRefError("UID FETCH失败")
    internaldate = _internaldate(response.metadata)
    message = BytesParser().parsebytes(response.raw)
    return {**reference.public_dict(), "subject": decode_str(message.get("Subject", "")) or "(无主题)",
            "sender": decode_str(message.get("From", "")), "date": message.get("Date", ""),
            "preview": extract_preview(message), "internaldate": internaldate.isoformat()}, internaldate
    ...
    try:
        ...
        with imap_connection(Credentials(email_addr, auth_code)) as mail:
            ...
            for reference in references:
                try:
                    item, internaldate = fetch_email_summary(mail, reference)
                    if not recent_cutoff or internaldate.astimezone(UTC8) >= recent_cutoff:
                        summaries.append((item, internaldate))
                except Exception as exc:
        ...
        result = {"status": "partial" if failed and summaries else "error" if failed and not summaries else "success",
                  "folder": "ALL" if all_folders else folder, "since": since, "before": before,
                  "recent": recent, "seen": seen, "total_matched": total_matched, "total": len(page),
                  "total_displayable": total_displayable, "has_more": has_more,
                  "emails": [item for item, _date in page]}
        if failed:
