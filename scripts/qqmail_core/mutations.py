...
from .confirmation import confirmation_matches, move_manifest
from .connections import imap_connection
from .imap_uid import refresh_capabilities, select_uid_section
from .folders import FolderError, choose_trash_folder, parse_list_response, quote_mailbox
    ...
    for reference in references:
        try:
            ...
            status, data = mail.uid(
                "FETCH", reference.uid,
                "(UID BODY.PEEK[HEADER.FIELDS (SUBJECT FROM DATE)]<0.16384>)",
            )
            header = (select_uid_section(data, reference.uid,
                                         "HEADER.FIELDS (SUBJECT FROM DATE)",
                                         maximum=16 * 1024)
                      if status == "OK" else None)
            if header is None:
                raise MailRefError("UID FETCH失败")
            message = BytesParser().parsebytes(header.raw)
            previews.append({**reference.public_dict(),
                             "subject": decode_header_value(message.get("Subject"), strict_charset=True) or "(无主题)",
                             "sender": decode_header_value(message.get("From"), strict_charset=True),
...
def main():
    ...
    try:
        ...
        references = parse_mailref_csv(args.src_folder, args.uidvalidity, args.mail_ids)
    ...
    return emit_json(move_emails(credentials.email, credentials.auth_code,
                                 [reference.uid for reference in references], args.src_folder,
