...
def _commit_without_overwrite(temporary: pathlib.Path, directory: pathlib.Path, requested: str, fd: int) -> pathlib.Path:
...
def _save_part(payload: bytes, encoding: str, directory: pathlib.Path, name: str, remaining: int) -> tuple[pathlib.Path, int, str | None]:
    ...
    try:
        ...
        destination = _commit_without_overwrite(temporary, directory, name, fd)
        published = True
    finally:
        try:
            os.close(fd)
...
def main():
    ...
    return emit_json(download_attachments(credentials.email, credentials.auth_code,
                                          [reference.uid for reference in references],
                                          args.folder, references[0].uidvalidity, args.dir, args.file))
