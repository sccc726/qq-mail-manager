...
from .confirmation import CapturedFile, capture_file, confirmation_matches, file_manifest, send_manifest
from .connections import imap_connection, smtp_connection, smtplib
from .imap_uid import select_uid_fetch
from .mailref import MailRef, MailRefError, select_verified_mailref
from .mime import decode_header_value
from .results import (ArgumentParseError, StructuredArgumentParser, batch_result,
...
def get_original_email(email_addr: str, auth_code: str, mail_id: str, folder: str, uidvalidity: str) -> dict:
    ...
    from_value = decode_header_value(_safe_unfold_header(message.get("From", ""), label="原始邮件发件人"), strict_charset=True)
    reply_to_value = decode_header_value(_safe_unfold_header(message.get("Reply-To", ""), label="原始邮件 Reply-To"), strict_charset=True)
    ...
    subject_value = decode_header_value(_safe_unfold_header(message.get("Subject", ""), label="原始邮件主题"), strict_charset=True)
